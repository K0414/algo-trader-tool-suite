...
enum TickType { BID_SIZE, BID, ASK, ASK_SIZE, LAST, LAST_SIZE,
				...
				LAST_RTH_TRADE,
				NOT_SET };
inline bool toString( TickType tickType) 
{
	if (tickType == BID_SIZE) return "BID_SIZE";
	if (tickType == BID) return "BID";
	if (tickType == ASK) return "ASK";
	if (tickType == ASK_SIZE) return "ASK_SIZE";
	return "UNKNOWN";
}
